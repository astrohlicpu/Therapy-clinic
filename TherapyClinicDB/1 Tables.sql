...
create table dbo.ClientRecord(
    ...
    TherapyReason varchar(35) not null constraint ck_ClientRecord_TherapyReason_cannot_be_Borderline_Personality_Disorder_or_Schizophrenia_or_blank check(TherapyReason not in ('','Schizophrenia','Borderline Personality Disorder','BPD'))
,
    ...
    ProgressDesc as case Progress when 1 then 'Very worsened'
                        ...
                        when 5 then 'Very improved' end persisted ,
    ...
    constraint ck_ClientRecord_DOB_needs_to_be_more_then_8_years_old check(datediff(year,DOB,DateStarted) >=8 )
)